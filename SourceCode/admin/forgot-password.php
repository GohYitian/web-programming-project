...
if (isset($_POST['reset'])) {
    ...
    $query = mysqli_query($con, "select ID from admin where email='$uname' ");
    $ret = mysqli_fetch_array($query);

    if ($ret > 0) {

        $sql = mysqli_query($con, "UPDATE admin SET password = '$newpassword ' where email ='$uname'");
        echo "<script>alert('Password successfully reset.');</script>";
        echo "<script>window.location.href ='login.php'</script>";
    } else {
        $_SESSION['errmsg'] = "Invalid username (email address)";
...
<head>
    ...
    <meta name="forget password" content="admin reset password">
    <meta name="CVBS" content="admin reset password">
